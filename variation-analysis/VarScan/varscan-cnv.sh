...
#$ -o $HOME/TNBC_Project/variation-analysis/VarScan/varscan-cnv-$JOB_ID.out

# send the job error messages to a file
#$ -e $HOME/TNBC_Project/variation-analysis/VarScan/varscan-cnv-$JOB_ID.err
...
# We are skipping the pileup lines because we alrady did this with the somatic mutation call 
...
java -jar /export/apps/VarScan.v2.4.2.jar copynumber patient_$PATIENT_ID-N.pileup patient_$PATIENT_ID-T.pileup patient_$PATIENT_ID

java -jar /export/apps/VarScan.v2.4.2.jar copyCaller patient_$PATIENT_ID.copynumber --output-file patient_$PATIENT_ID.copynumber.called
